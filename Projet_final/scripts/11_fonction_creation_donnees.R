...
espece <- function(chemin_acces){
  taxo_BD <- read.csv("tables_pour_SQL/taxonomie_BD.csv")
...
source <- function(chemin_acces){
  lepidopteres_final <- read.csv("data/lepido_final.csv")
	...
	write.csv(res.2.source, file="tables_pour_SQL/source.csv")
	return(res.2.source)
}
...
latitude <- function(chemin_acces){
  lepidopteres_final <- read.csv("data/lepido_final.csv")
	...
	write.csv(res.2.latitude, file="tables_pour_SQL/latitude.csv")
...
longitude <- function(chemin_acces){
  lepidopteres_final <- read.csv("data/lepido_final.csv")
	...
	write.csv(res.2.longitude, file="tables_pour_SQL/longitude.csv")
...
abbondance <- function(chemin_acces){
  lepidopteres_final <- read.csv("data/lepido_final.csv")
	...
	write.csv(res.2.abbondance, file="tables_pour_SQL/abbondance.csv")
...
observation <- function(chemin_acces){
  lepidopteres_final <- read.csv("data/lepido_final.csv")
	...
	write.csv(res.1.observation, file="tables_pour_SQL/observation.csv")
